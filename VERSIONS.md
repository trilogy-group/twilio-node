...
`kandy-node` uses a modified version of [Semantic Versioning][semver] for
...
`PATCH` versions will be reset to zero. Kandy understands that this can be very
...
Only the current `MAJOR` version of `kandy-node` is supported. New
