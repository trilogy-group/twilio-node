...
export interface ServiceContextUpdateOptions {
  ...
  friendlyName?: string;
  /** The URL we call using `inbound_method` when a message is received by any phone number or short code in the Service. When this property is `null`, receiving inbound messages is disabled. All messages sent to the Kandy phone number or short code will not be logged and received on the Account. If the `use_inbound_webhook_on_number` field is enabled then the webhook url defined on the phone number will override the `inbound_request_url` defined for the Messaging Service. */
  ...
  /** The URL we should call to [pass status updates](https://www.kandy.com/docs/sms/api/message-resource#message-status-values) about message delivery. */
  statusCallback?: string;
  /** Whether to enable [Sticky Sender](https://www.kandy.com/docs/sms/services#sticky-sender) on the Service instance. */
  stickySender?: boolean;
  /** Whether to enable the [MMS Converter](https://www.kandy.com/docs/sms/services#mms-converter) for messages sent through the Service instance. */
  mmsConverter?: boolean;
  /** Whether to enable [Smart Encoding](https://www.kandy.com/docs/sms/services#smart-encoding) for messages sent through the Service instance. */
  smartEncoding?: boolean;
  /**  */
  scanMessageContent?: ServiceScanMessageContent;
  /** Whether to enable [Fallback to Long Code](https://www.kandy.com/docs/sms/services#fallback-to-long-code) for messages sent through the Service instance. */
  fallbackToLongCode?: boolean;
  /** Whether to enable [Area Code Geomatch](https://www.kandy.com/docs/sms/services#area-code-geomatch) on the Service Instance. */
...
export class ServiceInstance {
  ...
  fallbackMethod: string;
  /**
   * The URL we call to [pass status updates](https://www.kandy.com/docs/sms/api/message-resource#message-status-values) about message delivery.
   */
  statusCallback: string;
  /**
   * Whether to enable [Sticky Sender](https://www.kandy.com/docs/sms/services#sticky-sender) on the Service instance.
