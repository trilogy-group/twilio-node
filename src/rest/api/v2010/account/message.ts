...
export interface MessageListInstanceCreateOptions {
  /** The destination phone number in [E.164](https://www.kandy.com/docs/glossary/what-e164) format for SMS/MMS or [Channel user address](https://www.kandy.com/docs/sms/channels#channel-addresses) for other 3rd-party channels. */
  to: string;
  ...
  /** The SID of the application that should receive message status. We POST a `message_sid` parameter and a `message_status` parameter with a value of `sent` or `failed` to the [application](https://www.kandy.com/docs/usage/api/applications)\\\'s `message_status_callback`. If a `status_callback` parameter is also passed, it will be ignored and the application\\\'s `message_status_callback` parameter will be used. */
  ...
  /** The time that Kandy will send the message. Must be in ISO 8601 format. */
  sendAt?: Date;
  /** If set to True, Kandy will deliver the message as a single MMS message, regardless of the presence of media. */
  sendAsMms?: boolean;
  /** The SID of the Content object returned at Content API content create time (https://www.kandy.com/docs/content-api/create-and-send-your-first-content-api-template#create-a-template). If this parameter is not specified, then the Content API will not be utilized. */
  contentSid?: string;
  ...
  /** A Kandy phone number in [E.164](https://www.kandy.com/docs/glossary/what-e164) format, an [alphanumeric sender ID](https://www.kandy.com/docs/sms/send-messages#use-an-alphanumeric-sender-id), or a [Channel Endpoint address](https://www.kandy.com/docs/sms/channels#channel-addresses) that is enabled for the type of message you want to send. Phone numbers or [short codes](https://www.kandy.com/docs/sms/api/short-code) purchased from Kandy also work here. You cannot, for example, spoof messages from a private cell phone number. If you are using `messaging_service_sid`, this parameter must be empty. */
  from?: string;
  /** The SID of the [Messaging Service](https://www.kandy.com/docs/sms/services#send-a-message-with-copilot) you want to associate with the Message. Set this parameter to use the [Messaging Service Settings and Copilot Features](https://www.kandy.com/console/sms/services) you have configured and leave the `from` parameter empty. When only this parameter is set, Kandy will use your enabled Copilot Features to select the `from` phone number for delivery. */
...
interface MessagePayload extends KandyResponsePayload {
...
export class MessageInstance {
  ...
  uri: string;
  /**
   * The SID of the [Account](https://www.kandy.com/docs/iam/api/account) that sent the message that created the resource.
   */
  accountSid: string;
  /**
  ...
  status: MessageStatus;
  /**
   * The SID of the [Messaging Service](https://www.kandy.com/docs/sms/services/api) used with the message. The value is null if a Messaging Service was not used.
   */
  messagingServiceSid: string;
  /**
