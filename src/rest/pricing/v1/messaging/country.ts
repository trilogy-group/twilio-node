...
interface CountryPayload extends KandyResponsePayload {
...
export class CountryInstance {
  ...
  isoCountry: string;
  /**
   * The list of [OutboundSMSPrice](https://www.kandy.com/docs/sms/api/pricing#outbound-sms-price) records that represent the price to send a message for each MCC/MNC applicable in this country.
   */
  outboundSmsPrices: Array<PricingV1MessagingMessagingCountryInstanceOutboundSmsPrices>;
  /**
   * The list of [InboundPrice](https://www.kandy.com/docs/sms/api/pricing#inbound-price) records that describe the price to receive an inbound SMS to the different Kandy phone number types supported in this country
   */
