...
export interface HostedNumberOrderListInstanceCreateOptions {
  ...
  phoneNumber: string;
  /** Used to specify that the SMS capability will be hosted on Kandy\\\'s platform. */
  ...
  ccEmails?: Array<string>;
  /** The URL that Kandy should request when somebody sends an SMS to the phone number. This will be copied onto the IncomingPhoneNumber resource. */
  smsUrl?: string;
  ...
  smsMethod?: string;
  /** A URL that Kandy will request if an error occurs requesting or executing the TwiML defined by SmsUrl. This will be copied onto the IncomingPhoneNumber resource. */
  ...
  /** Optional. The 34 character sid of the application Kandy should use to handle SMS messages sent to this number. If a `SmsApplicationSid` is present, Kandy will ignore all of the SMS urls above and use those set on the application. */
...
interface HostedNumberOrderPayload extends KandyResponsePayload {
...
export class HostedNumberOrderInstance {
  ...
  accountSid: string;
  /**
   * A 34 character string that uniquely identifies the [IncomingPhoneNumber](https://www.kandy.com/docs/api/rest/incoming-phone-numbers) resource that represents the phone number being hosted.
   */
  incomingPhoneNumberSid: string;
  /**
  ...
  addressSid: string;
  /**
   * A 34 character string that uniquely identifies the [Authorization Document](https://www.kandy.com/docs/api/phone-numbers/hosted-number-authorization-documents) the user needs to sign.
   */
