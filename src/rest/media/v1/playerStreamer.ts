...
export interface PlayerStreamerListInstanceCreateOptions {
  ...
  video?: boolean;
  /** The URL to which Kandy will send asynchronous webhook requests for every PlayerStreamer event. See [Status Callbacks](/docs/live/status-callbacks) for more details. */
  statusCallback?: string;
  /** The HTTP method Kandy should use to call the `status_callback` URL. Can be `POST` or `GET` and the default is `POST`. */
  statusCallbackMethod?: string;
  /** The maximum time, in seconds, that the PlayerStreamer is active (`created` or `started`) before automatically ends. The default value is 300 seconds, and the maximum value is 90000 seconds. Once this maximum duration is reached, Kandy will end the PlayerStreamer, regardless of whether media is still streaming. */
  maxDuration?: number;
}
/**
...
interface PlayerStreamerPayload extends KandyResponsePayload {
...
export class PlayerStreamerInstance {
  ...
  /**
   * The SID of the [Account](https://www.kandy.com/docs/iam/api/account) that created the PlayerStreamer resource.
   */
  accountSid: string;
  /**
