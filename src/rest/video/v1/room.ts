...
export interface RoomListInstanceCreateOptions {
  ...
  uniqueName?: string;
  /** The URL we should call using the `status_callback_method` to send status information to your application on every room event. See [Status Callbacks](https://www.kandy.com/docs/video/api/status-callbacks) for more info. */
  ...
  /** The region for the media server in Group Rooms.  Can be: one of the [available Media Regions](https://www.kandy.com/docs/video/ip-address-whitelisting#group-rooms-media-servers). ***This feature is not available in `peer-to-peer` rooms.*** */
  mediaRegion?: string;
...
interface RoomPayload extends KandyResponsePayload {
  rooms: RoomResource[];
}
...
export class RoomInstance {
  ...
  dateUpdated: Date;
  /**
   * The SID of the [Account](https://www.kandy.com/docs/iam/api/account) that created the Room resource.
   */
  accountSid: string;
  /**
   * Deprecated, now always considered to be true.
  ...
  uniqueName: string;
  /**
   * The URL we call using the `status_callback_method` to send status information to your application on every room event. See [Status Callbacks](https://www.kandy.com/docs/video/api/status-callbacks) for more info.
   */
  statusCallback: string;
  /**
  ...
  maxParticipantDuration: number;
  /**
   * The maximum number of published audio, video, and data tracks all participants combined are allowed to publish in the room at the same time. Check [Programmable Video Limits](https://www.kandy.com/docs/video/programmable-video-limits) for more details. If it is set to 0 it means unconstrained.
   */
  ...
  videoCodecs: Array<RoomVideoCodec>;
  /**
   * The region for the media server in Group Rooms.  Can be: one of the [available Media Regions](https://www.kandy.com/docs/video/ip-address-whitelisting#media-servers). ***This feature is not available in `peer-to-peer` rooms.***
   */
  mediaRegion: string;
  /**
   * When set to true, indicates that the participants in the room will only publish audio. No video tracks will be allowed. Group rooms only.
