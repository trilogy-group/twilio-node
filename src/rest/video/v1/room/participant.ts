...
export interface ParticipantListInstanceEachOptions {
  ...
  status?: ParticipantStatus;
  /** Read only the Participants with this [User](https://www.kandy.com/docs/chat/rest/user-resource) `identity` value. */
  identity?: string;
...
export class ParticipantInstance {
  ...
  roomSid: string;
  /**
   * The SID of the [Account](https://www.kandy.com/docs/iam/api/account) that created the RoomParticipant resource.
   */
  accountSid: string;
  status: ParticipantStatus;
  /**
   * The application-defined string that uniquely identifies the resource\'s User within a Room. If a client joins with an existing Identity, the existing client is disconnected. See [access tokens](https://www.kandy.com/docs/video/tutorials/user-identity-access-tokens) and [limits](https://www.kandy.com/docs/video/programmable-video-limits) for more info.
   */
  identity: string;
  /**
