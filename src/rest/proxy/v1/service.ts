...
export interface ServiceContextUpdateOptions {
  ...
  /** The URL we should call when an inbound call or SMS action occurs on a closed or non-existent Session. If your server (or a Kandy [function](https://www.kandy.com/functions)) responds with valid [TwiML](https://www.kandy.com/docs/voice/twiml), we will process it. This means it is possible, for example, to play a message for a call, send an automated text message response, or redirect a call to another Phone Number. See [Out-of-Session Callback Response Guide](https://www.kandy.com/docs/proxy/out-session-callback-response-guide) for more information. */
