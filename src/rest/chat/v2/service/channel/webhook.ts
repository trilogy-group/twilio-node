...
export interface WebhookContextUpdateOptions {
  ...
  /** The events that cause us to call the Channel Webhook. Used when `type` is `webhook`. This parameter takes only one event. To specify more than one event, repeat this parameter for each event. For the list of possible events, see [Webhook Event Triggers](https://www.kandy.com/docs/chat/webhook-events#webhook-event-trigger). */
  ...
  /** The SID of the Studio [Flow](https://www.kandy.com/docs/studio/rest-api/flow) to call when an event in `configuration.filters` occurs. Used only when `type` = `studio`. */
...
export interface WebhookListInstanceCreateOptions {
  ...
  /** A string that will cause us to call the webhook when it is present in a message body. This parameter takes only one trigger string. To specify more than one, repeat this parameter for each trigger string up to a total of 5 trigger strings. Used only when `type` = `trigger`. */
  "configuration.triggers"?: Array<string>;
  /** The SID of the Studio [Flow](https://www.kandy.com/docs/studio/rest-api/flow) to call when an event in `configuration.filters` occurs. Used only when `type` is `studio`. */
  "configuration.flowSid"?: string;
  /** The number of times to retry the webhook if the first attempt fails. Can be an integer between 0 and 3, inclusive, and the default is 0. */
  "configuration.retryCount"?: number;
}
...
interface WebhookPayload extends KandyResponsePayload {
...
export class WebhookInstance {
  ...
  sid: string;
  /**
   * The SID of the [Account](https://www.kandy.com/docs/iam/api/account) that created the Channel Webhook resource.
   */
  accountSid: string;
  /**
   * The SID of the [Service](https://www.kandy.com/docs/chat/rest/service-resource) the Channel Webhook resource is associated with.
   */
  serviceSid: string;
  /**
   * The SID of the [Channel](https://www.kandy.com/docs/chat/channels) the Channel Webhook resource belongs to.
   */
  channelSid: string;
  /**
