...
export interface SyncListItemContextUpdateOptions {
  ...
  ttl?: number;
  /** How long, [in seconds](https://www.kandy.com/docs/sync/limits#sync-payload-limits), before the List Item expires (time-to-live) and is deleted. */
  itemTtl?: number;
  /** How long, [in seconds](https://www.kandy.com/docs/sync/limits#sync-payload-limits), before the List Item\\\'s parent Sync List expires (time-to-live) and is deleted. This parameter can only be used when the List Item\\\'s `data` or `ttl` is updated in the same request. */
...
export interface SyncListItemListInstanceCreateOptions {
  ...
  /** How long, [in seconds](https://www.kandy.com/docs/sync/limits#sync-payload-limits), before the List Item expires (time-to-live) and is deleted. */
  itemTtl?: number;
  /** How long, [in seconds](https://www.kandy.com/docs/sync/limits#sync-payload-limits), before the List Item\\\'s parent Sync List expires (time-to-live) and is deleted. */
  collectionTtl?: number;
}
/**
...
interface SyncListItemPayload extends KandyResponsePayload {
...
export class SyncListItemInstance {
  ...
  index: number;
  /**
   * The SID of the [Account](https://www.kandy.com/docs/iam/api/account) that created the List Item resource.
   */
  accountSid: string;
  /**
