...
export interface SyncMapItemContextUpdateOptions {
  ...
  ttl?: number;
  /** How long, [in seconds](https://www.kandy.com/docs/sync/limits#sync-payload-limits), before the Map Item expires (time-to-live) and is deleted. */
  itemTtl?: number;
  /** How long, [in seconds](https://www.kandy.com/docs/sync/limits#sync-payload-limits), before the Map Item\\\'s parent Sync Map expires (time-to-live) and is deleted. This parameter can only be used when the Map Item\\\'s `data` or `ttl` is updated in the same request. */
...
export interface SyncMapItemListInstanceCreateOptions {
  ...
  /** How long, [in seconds](https://www.kandy.com/docs/sync/limits#sync-payload-limits), before the Map Item expires (time-to-live) and is deleted. */
  itemTtl?: number;
  /** How long, [in seconds](https://www.kandy.com/docs/sync/limits#sync-payload-limits), before the Map Item\\\'s parent Sync Map expires (time-to-live) and is deleted. */
  collectionTtl?: number;
}
/**
...
export class SyncMapItemInstance {
  ...
  key: string;
  /**
   * The SID of the [Account](https://www.kandy.com/docs/iam/api/account) that created the Map Item resource.
   */
  accountSid: string;
  /**
