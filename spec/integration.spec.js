...
describe("kandy", function () {
  it("should set the account sid and auth token", function () {
    var client = kandy(accountSid, token);
  ...
  it("should provide list shorthand alias", function () {
    var client = new kandy(accountSid, token);
  ...
  it("should provide each for integration", function () {
    var client = new kandy.Kandy(accountSid, token);
  ...
  it("should set Kandy and HTTP client auto-retry with exponential backoff properties", function () {
    var client = new kandy.Kandy(accountSid, token, {
      autoRetry: true,
      maxRetries: 5,
    });
